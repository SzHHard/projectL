...
const interceptor = (store) => {
    ...
    instance.interceptors.response.use((config) => {
       ...
       // debugger;
        ...
        if (err.response.status === 401 && err.config && err.config.url !== '/auth/refresh') {  
            originalConfig._isRetry = true; // не удалось менять 

            try {
             const res = await instance.get('/auth/refresh')  // я делаю запрос и получаю ответ с тем же кодом ошибки, но мне нужно дать программе понять, что второй раз лопатить не надо
